...
#'  such that \code{100 * conf.level}\% of the parameter's posterior distribution
...
tidy.brmsfit <- function(x, parameters = NA,
  ...
  if (use_effects) {
    ...
    if ("ran_vals" %in% effects) {
      ...
      res_list$ran_vals <-
        data_frame(
          group = plyr::laply(vals, function(v) {
            v[[2]]
          }),
          term = plyr::laply(vals, function(v) {
            v[[4]]
          }),
          level = plyr::laply(vals, function(v) {
            v[[3]]
          })
        )
    }
