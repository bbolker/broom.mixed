...
if (suppressPackageStartupMessages(require("brms", quietly = TRUE))) {
  ...
  test_that("component tags stripped from brms models", {
    expect_equal(
      c(table(zz2[["term"]])),
      c(
        `(Intercept)` = 2L, minedno = 2L, `sd__(Intercept)` = 2L,
        sppDESML = 1L,
  ...
  test_that("multi-component brms models", {
    check_tidy(
      zz3, 8, 9,
      c(
        "response", "effect", "component", "group",
  ...
  sleepstudy.levels <- rep(c(
    "308", "309", "310", "330", "331", "332", "333", "334", "335", "337",
    "349", "350", "351", "352", "369", "370", "371", "372"
  ), 2)
